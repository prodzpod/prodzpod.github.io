...
function isEmptyOrWhitespace(str) {
    return (!str && str !== 0 && str !== false) || !str.toString().trim().length;
}
...
function utob(str) {
    return btoa(Array.from(new TextEncoder().encode(str), x => String.fromCodePoint(x)).join(''));
}

function btou(str) {
    return new TextDecoder().decode(Uint8Array.from(atob(str), x => x.codePointAt(0)));
}
...
function seatableToDate(str) {
    if (str.includes("+")) str = str.split('+')[0];
    if (str.includes(":")) return new Date(new Date(str).getTime() - new Date("1970-1-1 0:0").getTime());
    else new Date(str);
}
...
function foreachElement(cl, fn) { for (let e of Array.from(document.getElementsByClassName(cl))) fn(e); }
...
function send(method, query, body, cb) {
    try {
        ...
        xhr.addEventListener('load', (e) => { cb((xhr.status == 204 || xhr.status == 404) ? '' : JSON.parse(xhr.responseText), xhr.status); });
...
function send_post(query, cb) {
    return send("POST", query, {'id': window.localStorage.getItem('id') ?? "", 'password': btou(btou(btou(window.localStorage.getItem('password') ?? "")))}, cb);
}
